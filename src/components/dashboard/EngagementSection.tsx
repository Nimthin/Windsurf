...
const EngagementSection: React.FC<EngagementSectionProps> = ({ 
  ...
  const instagramImageEngagementData = useMemo(() => {
    ...
    const processedBrandData = selectedBrands.map(brand => {
      ...
      const totalLikes = imagePosts.reduce((sum, post) => sum + (post.likesCount || 0), 0);
      const totalComments = imagePosts.reduce((sum, post) => sum + (post.commentsCount || 0), 0);
      return {
        brandName: brand,
    ...
    if (platform === 'Instagram') {
      ...
      return {
        ...
        datasets: [ // Assuming original structure for TikTok bar chart is preferred
          {
            label: 'Avg. Likes',
            data: processedBrandData.map(data => data!.metrics[0].value),
          ...
          {
            label: 'Avg. Comments',
            data: processedBrandData.map(data => data!.metrics[1].value),
          ...
          {
            label: 'Avg. Shares',
            data: processedBrandData.map(data => data!.metrics[2].value),
          ...
          },
          {
            ...
            data: processedBrandData.map(data => data!.metrics[3].value),
  ...
  const commonChartOptions: ChartOptions<'bar'> | ChartOptions<'line'> = { // Removed Pie from common options type
    responsive: true,
  ...
  return (
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {platform === 'Instagram' && (
          <div className={`p-4 rounded-lg shadow ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'} lg:col-span-2`}>
            ...
            <div className="h-80">
              {!hasData || instagramImageEngagementData.labels.length === 0 ? (
                ...
                <Line data={instagramImageEngagementData} options={commonChartOptions as ChartOptions<'line'>} />
              )}
            </div>
          </div>
        )}
        ...
        <div className={`p-4 rounded-lg shadow ${darkMode ? 'bg-gray-700/50' : 'bg-gray-50'} ${platform === 'Instagram' ? 'lg:col-span-2' : 'lg:col-span-2'}`}>
          <h3 className={`text-md font-semibold mb-3 ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>
          ...
          <div className="h-80">
            {!hasData || videoEngagementData.labels.length === 0 ? (
              ...
              platform === 'Instagram' ?
                <Line data={videoEngagementData} options={commonChartOptions as ChartOptions<'line'>} /> :
                <Bar data={videoEngagementData} options={commonChartOptions as ChartOptions<'bar'>} />
            )}
          </div>
        </div>
